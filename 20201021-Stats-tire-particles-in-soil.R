...
#
#        Author of this script:  Masahiro Ryo (last update: 21.10.2020)
#####################################################################
...
df <- read.csv("data_hanna.csv", header = TRUE, sep = ";", dec = ".") %>% 
...
response.label <- list("shoot biomass weight (mg)", "root biomass weight (mg)",
                       "litter weight loss (%)", 
                       c(as.expression(bquote('soil respiration (?g' ~CO[2]~ g^-1~h^-1*')'))),
                       "soil pH")
...
for(i_response in response.names){
  ...
  ggplots[[i_response]] <-
    ...
    geom_point(data=df %>% filter(response==i_response), 
               aes(x= concentration_tp, y= value, alpha=0.8)) +
...
ggplots[["shootweight"]] + ggplots[["rootweight"]] +
...
(ggplots[["shootweight"]]/ ggplots[["rootweight"]] / plot_spacer()) | # first column
  (ggplots[["litterweightloss"]] / ggplots[["CO2"]] / ggplots[["pH"]])+ # second column
  plot_annotation(tag_levels = 'A')
